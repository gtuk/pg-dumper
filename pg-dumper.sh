...
if ! [ -x "$(command -v gzip)" ]; then
  echo "gzip is not installed or not executable"
  exit 1
fi
...
if [ "$cluster_wide" = true ] && ! [ -x "$(command -v pg_dumpall)" ]; then
  echo "pg_dumpall is not installed or not executable"
  exit 1
fi

if [ "$cluster_wide" = false ] && ! [ -x "$(command -v pg_dump)" ]; then
  echo "pg_dump is not installed or not executable"
  exit 1
fi
...
if [ "$encryption" = true ]; then
  if [ "$cluster_wide" = true ]; then
    if PGPASSWORD=$password pg_dumpall -U "$user" -h "$host" -p "$port" | gzip | gpg --batch --passphrase "$backup_password" -c -o "$location"/"$name"; then backup_result=0; else backup_result=1; fi
  elif [ "$db" ]; then
   if PGPASSWORD=$password pg_dump -d "$db" -U "$user" -h "$host" -p "$port" | gzip | gpg --batch --passphrase "$backup_password" -c -o "$location"/"$name"; then backup_result=0; else backup_result=1; fi
  else
    echo "Backup failed. Db parameter needs to when cluster_wide parameter is false"
  fi
elif [ "$encryption" = false ]; then
  if [ "$cluster_wide" = true ]; then
     if PGPASSWORD=$password pg_dumpall -U "$user" -h "$host" -p "$port" | gzip > "$location"/"$name"; then backup_result=0; else backup_result=1; fi
  elif [ "$db" ]; then
    if PGPASSWORD=$password pg_dump -d "$db" -U "$user" -h "$host" -p "$port" | gzip > "$location"/"$name"; then backup_result=0; else backup_result=1; fi
  else
